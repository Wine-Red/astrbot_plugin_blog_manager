    ...
    def _build_prompt(
        ...
        *,
        ...
        prompt = (
            ...
            "- 不要编造看似真实但无法访问的图片 URL；图片 URL 必须来自搜索结果、用户提供链接或可直接确认的公开页面。\n"
            "- 只有在确实找不到可靠图片时，才允许 `image` 返回空字符串、`images` 返回空数组，并在正文中说明“未找到可靠公开图片来源”。\n"
            ...
            "- 来源链接只能使用搜索工具返回的 URL、用户提供的 URL，或你能从公开网页结果中直接确认的 URL。\n"
            "- 禁止根据公司名、文章标题或记忆自行拼接 URL；禁止使用 example.com、test.com、localhost 等占位链接。\n"
            "- 不要编造来源；来源不足时要明确说明“公开来源有限”，并降低结论确定性，不要强行添加链接。\n"
