    ...
    def _build_prompt(
        ...
        *,
        ...
        prompt = (
            ...
            "- `image` 用作文章封面，优先选择与主题直接相关、可公开访问、视觉质量稳定的图片 URL。\n"
            "- `images` 用作正文配图素材，建议 1 到 3 张；每张图的 `alt` 必须具体描述图中内容和对应章节，不要只写“配图”。\n"
            "- 如果能获取官方产品截图、架构图、发布页首图或可信媒体图片，应优先使用；不要使用与主题无关的通用图库图。\n"
            "- 不要编造看似真实但无法访问的图片 URL；没有可靠图片时，`image` 返回空字符串，`images` 返回空数组。\n"
            "- 如果正文中引用图片，必须使用标准 Markdown 图片语法，并让 alt 文本可读、可检索。\n\n"
            "文章质量要求：\n"
            ...
            "- 如果主题涉及新闻、产品、模型、版本、法规、价格、榜单或其他可能变化的信息，必须基于可用搜索能力核对事实，并在正文提供 Markdown 来源链接。\n"
            "- 不要编造来源；来源不足时要明确说明“公开来源有限”，并降低结论确定性。\n"
